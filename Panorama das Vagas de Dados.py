...
import pandas as pd
from Processamento import identificar_categoria, padronizar_ferramentas, padronizar_xp, padronizar_cargo
...
with coluna4.container(border=True):    
    ...
    make_bar_with_no_slice(soft_skills,'soft_skills','Soft Skills mais Demandas para Profissionais de Dados',15,13,20)
...
with coluna5.container(border=True):
    ...
    make_pie(vagas,'xp','Proporção da Senioridade nas Oportunidades',22)

with coluna6.container(border=True):

    make_bar_with_no_slice(vagas,'estado','Ranking dos Estados com Mais Vagas',15,13,22)
...
with coluna8.container(border=True):
    ...
    make_bar_with_no_slice(metodologia_trabalho,'metodologia_trabalho','Principais Metodologias Ágeis em Dados',15,15,18)
